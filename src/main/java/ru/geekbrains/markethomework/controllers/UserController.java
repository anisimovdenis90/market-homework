...
import ru.geekbrains.markethomework.exceptions.RegistrationError;
...
public class UserController {
    ...
    public ResponseEntity<?> saveNewUser(@RequestBody @Validated SystemUserDto systemUserDto, BindingResult bindingResult) {
        ...
        if (userService.isPresent(systemUserDto.getUsername())) {
            return new ResponseEntity<>(new RegistrationError(String.format("Username %s already exist", systemUserDto.getUsername())),
                    HttpStatus.BAD_REQUEST);
        }

        if (!systemUserDto.getPassword().equals(systemUserDto.getConfirmationPassword())) {
            return new ResponseEntity<>(new RegistrationError("Password and confirmation password isn't equal"),
                    HttpStatus.BAD_REQUEST);
        }
